...
//   manifest + chunks to R2; sets the computer's DO head (no wake); the response
//   also carries the Better Auth `Set-Cookie` session header.
//
...
export async function writeSeedTree(store: R2Bucket): Promise<{ manifest: string; files: string[] }> {
  const enc = new TextEncoder();
  const entries: ManifestEntry[] = [];

  for (const dir of dirsOf(Object.keys(SEED_TREE))) {
    entries.push({
      path: dir,
      kind: 'dir',
      mode: 0o040755,
  ...
  for (const [path, contents] of Object.entries(SEED_TREE)) {
  ...
  return { manifest: manifestId, files: Object.keys(SEED_TREE).sort() };
}
...
export async function runSeed(
  ...
  // Write the manifest + chunks to R2.
  const { manifest, files } = await writeSeedTree(env.STORE);
  ...
  // Seed the DO head without waking (spec 8.4).
